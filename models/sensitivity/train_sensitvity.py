...
from models.sensitivity.espirit import espirit, espirit_proj, ifft
...
class DataTransform:
    ...
    def __call__(self, kspace_np, target, attrs, fname, slice):
        """
        Args:
        ...
        """
        kspace = transforms.to_tensor(kspace_np)
        ...
        return masked_kspace, fname, slice, image, target, mean, std, attrs['norm'].astype(np.float32)
...
def train_epoch(args, epoch, model, data_loader, optimizer, writer):
    ...
    for iter, data in enumerate(data_loader):
        ...
        masked_kspace, fname, slice, input, target, mean, std, norm = data
        ...
        if iter % args.report_interval == 0:
            time_inner = time.perf_counter() - start_iter
            time_loading = (time.perf_counter() - end_iter) - time_inner
            logging.info(
                f'Epoch = [{epoch:3d}/{args.num_epochs:3d}] '
                f'Iter = [{iter:4d}/{len(data_loader):4d}] '
                f'Loss = {loss.item():.4g} Avg Loss = {avg_loss:.4g} '
                f'Training time = {time_inner:.4f}s '
                f'Loading time = {time_loading:.4f}s'
            )
...
def create_arg_parser():
    ...
    parser.add_argument('--exp-name', type=str, default='unet',
...
if __name__ == '__main__':
    ...
    args.exp_dir = pathlib.Path('checkpoints/' + exp_name)
