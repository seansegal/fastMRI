...
from models.uncertainty.unet_ranking_baseline.unet_model_with_confidence import UnetModelWithConfidence
from models.mri_model import MRIModel

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

# HACK: Global variable to configure regularization.
...
class RankingBaseline(MRIModel):
    ...
    def train_step(self, model, data, device):
        ...
        loss_matrix = F.l1_loss(output, target, reduction='none')
        batch_losses = loss_matrix.sum(dim=2).sum(dim=1) / loss_matrix[0].numel()
        weights = F.softmax(confidence, dim=0)
        loss = torch.sum(weights * batch_losses.unsqueeze(1))
        if self.entropy_coeff > 0:
            entropy = torch.distributions.categorical.Categorical(probs=weights.squeeze(1)).entropy()
