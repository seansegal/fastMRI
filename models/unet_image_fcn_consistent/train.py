...
from models.unet_image_fcn_consistent.unet_model import UnetModel
...
MULTI = float(os.environ['MULTI']) if 'MULTI' in os.environ else 0.0
...
class DataTransform:
    ...
    def __call__(self, kspace, target, attrs, fname, slice):
        ...
        image_crop = transforms.complex_center_crop(image, (self.resolution, self.resolution))
        _, mean, std = transforms.normalize_instance_complex(image_crop, eps=1e-11)
        ...
        return image, target_train, target_kspace_train, mean, std, mask, mean_abs, std_abs, target_inference, attrs['max'], attrs['norm'].astype(np.float32)
...
def project_to_consistent_subspace(output, input, mask):
    ...
    return transforms.ifft2(new_kspace), new_kspace


def generate(generator, data, device):

    input, _, _, mean, std, mask, _, _, _, _, _ = data
    ...
    output_consistent, output_kspace = project_to_consistent_subspace(output_network, input, mask)
    ...
    return output_consistent, output_network, output_kspace
...
def train_step(model, data, device):
    ...
    output_consistent, output_network, output_kspace = generate(model, data, device)
    if RENORM:
...
def inference(model, data, device):
    with torch.no_grad():
        output, _, _ = generate(model, data, device)
        _, _, _, _, _, _, _, _, target, _, _ = data
        target = target.to(device)
