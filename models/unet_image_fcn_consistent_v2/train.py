...
from models.unet_image_fcn_consistent_v2.unet_model import UnetModel
...
class DataTransform:
    ...
    def __call__(self, kspace, target, attrs, fname, slice):
        ...
        if RENORM:
            target_train = transforms.normalize(target_train, mean, std)
            
        if CLAMP:
            # image = image.clamp(-6, 6)
            target_train = target_train.clamp(-6, 6)
    
        return image, target_train, mean, std, mask, target, attrs['max'], attrs['norm'].astype(np.float32)
...
def train_step(model, data, device):
    _, target, mean, std, _, _, data_range, norm = data
    ...
    if RENORM:
        ...
        output_consistent = transforms.normalize(output_consistent, mean, std)
        output_network = transforms.normalize(output_network, mean, std)
